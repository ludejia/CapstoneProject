...
The next step is applyting different model to this dataset to see which model has the best performance. To achieve this, we will use 4 folder cross validation method and repeat this process 10 times on each model so we can get the 40 prediction accuracy results for each model to find out which model has the best accuracy. Let's start with the linear model.
...
eva_XGBoost <- train(log_Price ~., data = house,
                  ...
                  trControl=custom)
```
Below we can fing that XGBoost have the best performance among all the models.
```{r}
...
Comparing XGBoost with Random Forest, t.test shows a siginicant difference. 
```{r}
t.test(mev_Forest$RMSE,mev_XGBoost$RMSE)
```
...
log helps. 
tree model slow to build but better performance.
